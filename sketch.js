...
var floor1, floor2;
var score = 0;
var coinSound, gameOverSound, winSound;
...
function setup() {
  ...
  cop.addAnimation("copLeft", copLeft);
  cop.addAnimation("copLeftRun", copLeftRunning);
  cop.scale = 0.4;
  cop.velocityX = 3;
...
function draw() {
  ...
  if(gameState === "play"){
    //cop.bounceOff(edges);
    if(cop.isTouching(edges[1])){
      cop.changeAnimation("copLeftRun", copLeftRunning);
      cop.velocityX = -5;
    }
    
    if(cop.isTouching(edges[0])){
      cop.changeAnimation("copRun", copRunning);
      cop.velocityX = 5;
    }
